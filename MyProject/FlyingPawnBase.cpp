...
void AFlyingPawnBase::Tick(float DeltaTime)
{
	...
	//!!Replicate Final Result to Server (only as the client) 
		...
		FQuat pitch_yaw_for_server_quat = MainBody->GetRelativeRotation().Vector().ToOrientationQuat();
		FVector2D pitch_yaw_for_server = FVector2D(MainBody->GetRelativeRotation().Pitch, MainBody->GetRelativeRotation().Yaw);
		this->PassTransformToServer(pitch_yaw_for_server, MainBody->GetRelativeRotation().Roll);
...
void AFlyingPawnBase::PassTransformToServer_Implementation(FVector2D world_facing_pitch_yaw, float relative_roll_amount)
{
	//MainBody->SetSimulatePhysics(false);
	//Intuition is to make it so the server replicates out raw positional data
	//  rather than trying to simulate physics which creates really bad stuttering.
	...
	this->SIM_newest_pitch_yaw_goal = world_facing_pitch_yaw;
	OnRep_SIM_newest_pitch_yaw_goal();

	this->SIM_newest_roll_goal = relative_roll_amount;
	OnRep_SIM_newest_roll_goal();
...
void AFlyingPawnBase::HandleMovementOnSimulatedClient(float delta_time)
{
	MainBody->SetWorldRotation(FRotator(SIM_newest_pitch_yaw_goal.X, SIM_newest_pitch_yaw_goal.Y, SIM_newest_roll_goal));
	//Interpolate towards goals (rotation and translation)

	//Add latest known inputs (rotation and translation) 

	//Reduce magnitude of latest inputs to prevent them from growing stale. 
}



void AFlyingPawnBase::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	//Variables to share only with simulated actors
	DOREPLIFETIME_CONDITION(AFlyingPawnBase, SIM_newest_pitch_yaw_goal, COND_SimulatedOnly);
	DOREPLIFETIME_CONDITION(AFlyingPawnBase, SIM_newest_roll_goal, COND_SimulatedOnly);
	DOREPLIFETIME_CONDITION(AFlyingPawnBase, SIM_last_rotator_input, COND_SimulatedOnly);
	DOREPLIFETIME_CONDITION(AFlyingPawnBase, SIM_newest_position_goal, COND_SimulatedOnly);
	DOREPLIFETIME_CONDITION(AFlyingPawnBase, SIM_last_velocity_input, COND_SimulatedOnly);
}



void AFlyingPawnBase::OnRep_SIM_newest_pitch_yaw_goal()
{
...
void AFlyingPawnBase::OnRep_SIM_newest_roll_goal()
{
...
void AFlyingPawnBase::OnRep_SIM_last_rotator_input()
{
	//if (GetLocalRole() == ROLE_SimulatedProxy)
	//{
	//	GEngine->AddOnScreenDebugMessage(9910, 5, FColor::Purple, FString::Printf(TEXT("9910 recieved new rotator from serrver: %s"), *SIM_last_rotator_input.ToString()));
	//}
...
void AFlyingPawnBase::OnRep_Sim_newest_position_goal()
{
...
void AFlyingPawnBase::OnRep_SIM_last_velocity_input()
{
