...
void AFlyingPawnBase::PassRotationToServer_Implementation(FRotator rotational_velocity, FRotator newest_rotational_goal)
{
...
void AFlyingPawnBase::PassTranslationToServer_Implementation(FVector world_velocity, FVector reported_world_location)
{
...
//Simulate Translation
	MainBody->SetPhysicsLinearVelocity(this->SIM_last_velocity_input);
...
//Simulate Rotation
	...
	if (this->SIM_last_rotational_velocity.Equals(FRotator(0), 0.0001f)) 
	{
		...
		if (this->GetActorRotation().Equals(this->SIM_last_rotational_goal, 0.01f)) //could make this more efficient by expressing rotational velocity as a scalar value because we are only using it for it's magnitude in this case 
		{
		}
...
void AFlyingPawnBase::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION(AFlyingPawnBase, SIM_last_rotational_velocity, COND_SimulatedOnly);
	DOREPLIFETIME_CONDITION(AFlyingPawnBase, SIM_last_rotational_goal, COND_SimulatedOnly);

	DOREPLIFETIME_CONDITION(AFlyingPawnBase, SIM_last_position_goal, COND_SimulatedOnly);
...
void AFlyingPawnBase::OnRep_SIM_last_rotational_goal()
{
	...
	DrawDebugLine(GetWorld(), this->GetActorLocation(), this->GetActorLocation() + this->SIM_last_rotational_goal.Vector() * 1000, FColor::Emerald, false, 0.5f, 0U, 25.0f);
}



void AFlyingPawnBase::OnRep_SIM_last_position_goal()
{
	...
	FVector simulated_location = this->GetActorLocation();
...
void AFlyingPawnBase::OnRep_SIM_last_velocity_input()
{
	...
	GetWorldTimerManager().SetTimer(this->SIM_begin_decay_velocity_handle, this, &AFlyingPawnBase::SIMEnableDecayVelocity, 0.1f, false); //set a non-looping timer to set decay velocity flag to true
}



void AFlyingPawnBase::SIMEnableDecayVelocity(){
